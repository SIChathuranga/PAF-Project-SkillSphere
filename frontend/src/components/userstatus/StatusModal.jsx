...
const StatusModal = ({ isOpen, onClose, status, onEdit, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="status-modal" role="dialog" aria-modal="true">
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative transform overflow-hidden rounded-lg bg-white shadow-xl transition-all max-w-2xl w-full">
          <div className="absolute right-0 top-0 p-4">
            <button onClick={onClose} className="rounded-full p-1 hover:bg-gray-100 transition-colors" aria-label="Close modal">
          ...
          <div className="p-6">
            ...
            {status.imageUrl && (
              <div className="mt-4">
                <img src={status.imageUrl} alt="Status" className="rounded-lg max-h-[500px] w-full object-cover" />
              </div>
            )}
            {(onEdit || onDelete) && (
              <div className="mt-6 flex justify-end space-x-3 border-t pt-4">
                {onEdit && (
                  <Button variant="secondary" onClick={() => { onEdit(status); onClose(); }} icon={<PencilIcon size={16} />}>
                    Edit
                  </Button>
                )}
                {onDelete && (
                  <Button variant="outline" onClick={() => { onDelete(status.id); onClose(); }} icon={<TrashIcon size={16} />} className="text-red-600 border-red-600 hover:bg-red-50">
                    Delete
                  </Button>
                )}
              </div>
